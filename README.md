...
## 目前有群友实测，2k电阻下最大vddq反而降低了，所以大伙儿请勿轻易尝试  
#### 方法1. 
修改配置电阻Rtop的阻值，将其从1.55K欧姆调整为1.9K欧姆，则可实现1.55v的vddq，且不需要修改bios选项,
此时bios内存vddq电压的1.35V即为1.55V,同时原本的1.23v档位的内存电压也会提高。  
1.9k欧姆的电阻也可以替换成2k欧姆的电阻，这样最高内存电压会变成1.59v，这个就根据自己内存情况来调整阻值就行。  
![电路图](pics/Rtop.jpg  )     
#### 方法2.
补齐三极管和对应的电阻，电阻阻值未知，但从分析来看可能也是14K欧姆的；此时方法1中的电阻应该也要改，不然最大电压可能还是1.35V（**每个三极管导通的作用像是往Rtop并联电阻，如果不增大Rtop，会导致原来的1.23v档实际vddq电压低于1.2v**）  
另外还有个元件在空的三极管左边，感觉是滤波电容。  
